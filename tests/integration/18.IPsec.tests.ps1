...
Describe "Edge IPsec" {
    ...
    Context "ipsec" {
        ...
        it "Can retrieve IPsec Config" {
            ...
            $ipsec.sites | should be $null
            $ipsec.global | should not be $null
            $ipsec.logging.enable | should be false
            $ipsec.logging.loglevel | should be "info"
        }
        ...
        it "Enable logging" {
            #Enable logging
            Get-NsxEdge $ipsecedge1name | Get-NsxIPsec | Set-NsxIPsec -EnableLogging
            #Check ipsec logging
            (Get-NsxEdge $ipsecedge1name | Get-NsxIPsec).logging.enable | should be true
        }
        ...
        it "Remove IPsec Config" {
            ...
            $ipsec = Get-NsxEdge $ipsecedge1name | Get-NsxIPsec
            $ipsec.enabled | should be false
            $ipsec.cacheSize | should be 16
            $ipsec.ipsecViews.ipsecView.forwarders | should be $null
            $ipsec.logging.enable | should be false
            $ipsec.logging.loglevel | should be "info"
        }
