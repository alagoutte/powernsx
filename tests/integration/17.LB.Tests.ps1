...
Describe "Edge LB" {
    ...
    BeforeAll {
        ...
        $script:lb_edge_name = "pester-e-lbedge"
        $script:lb_uplink_ls_name = "pester_e_ls1"
        $script:lb_uplink_ip1 = "1.1.1.1"
        $script:lb_uplink_vip1 = "1.1.1.2"
        $script:Password = "VMware1!VMware1!"

        #Create Logical Switch
        $script:lb_uplink_ls = Get-NsxTransportZone -LocalOnly | select -first 1 | New-NsxLogicalSwitch $lb_uplink_ls_name

        #Create Edge Interface
        $vnic0 = New-NsxEdgeInterfaceSpec -index 0 -Type uplink -Name "vNic0" -ConnectedTo $lb_uplink_ls -PrimaryAddress $lb_uplink_ip1 -SecondaryAddress $lb_uplink_vip1 -SubnetPrefixLength 24

        #Create Edge
        $script:sslEdge = New-NsxEdge -Name $lb_edge_name -Interface $vnic0 -Cluster $cl -Datastore $ds -password $password -enablessh -hostname $lb_edge_name

    }

    Context "Load Balancing (Global)" {

        #Group related tests together.

        it "Enable Load Balance" {

            Get-NsxEdge $lb_edge_name | Get-NsxLoadBalancer | Set-NsxLoadBalancer -Enabled

            #can make multiple assertions to improve test value.
            (Get-NsxEdge $lb_edge_name | Get-NsxLoadBalancer).enabled | should be true
        }
    ...
    AfterAll {
        ...
        #get-nsxedge $lb_edge_name | remove-nsxedge -confirm:$false
        start-sleep 5

        #Get-NsxLogicalSwitch $lb_uplink_ls_name | Remove-NsxLogicalSwitch -Confirm:$false
